...
static inline void print_usage_exit(void) {
	printf("Usage: print-to-led-mmap <string>\n");
	exit(EXIT_SUCCESS);
}
...
void write_char_pixel_at_coords(const char c, int y, int x, struct smart_led *led) {
	...
	// This has better statistics but incorrect result
	// struct smart_led tmp;
	//
	// tmp.state = led_state;
	// tmp.color[0] = 255;
	// tmp.color[1] = 255;
	...
	// tmp.color[3] = 255;
	//
	// *(volatile uint64_t *)led = *(uint64_t *)&tmp;
	//
}
